...
def prop_labels(labels, k, neighbor_labels, ancestor_dict, info_content, y_pred_scores, a):
    """
    Voting algorithm that adds info-content weighted votes for ancestors of relevant GO labels (NO distance weights)
...
##                print(indices)
                ...
                for idx in indices:
                    y_pred_scores[a, idx] = y_pred_scores[a, idx] + 1 * info_content[0, idx]

    

def unweighted_complex_KNN(X_train, Y_train, X_test, labels, k, ancestor_dict, info_content):
    """
    ...
    for a in range(X_test.shape[0]): # iterate over datapoints adding votes where neighbors have 1 with GO propagation
        ...
        prop_labels(labels, k, neighbor_labels, ancestor_dict, info_content, y_pred_scores, a)
            
        y_pred_scores[a,:] = y_pred_scores[a,:]/(np.max(y_pred_scores[a,:]+ 1e-8)) # Normalizing each row's scores so no score exceeds 1
...
val_y_scoresk1_unweighted = unweighted_complex_KNN(X_train, Y_train, X_val, labels, 1, ancestor_dict, info_content)
val_y_scoresk1_unweighted.to_csv('IC_val_y_scoresk1_unweighted_max_normalization.csv', index=False)

val_y_scoresk2_unweighted = unweighted_complex_KNN(X_train, Y_train, X_val, labels, 2, ancestor_dict, info_content)
val_y_scoresk2_unweighted.to_csv('IC_val_y_scoresk2_unweighted_max_normalization.csv', index=False)

val_y_scoresk3_unweighted = unweighted_complex_KNN(X_train, Y_train, X_val, labels, 3, ancestor_dict, info_content)
val_y_scoresk3_unweighted.to_csv('IC_val_y_scoresk3_unweighted_max_normalization.csv', index=False)

val_y_scoresk5_unweighted = unweighted_complex_KNN(X_train, Y_train, X_val, labels, 5, ancestor_dict, info_content)
val_y_scoresk5_unweighted.to_csv('IC_val_y_scoresk5_unweighted_max_normalization.csv', index=False)

val_y_scoresk10_unweighted = unweighted_complex_KNN(X_train, Y_train, X_val, labels, 10, ancestor_dict, info_content)
val_y_scoresk10_unweighted.to_csv('IC_val_y_scoresk10_unweighted_max_normalization.csv', index=False)
